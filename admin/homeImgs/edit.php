...
$sql = "select * from home_imgs where id = $id";
$result = mysqli_query($link, $sql);
$arr_img = mysqli_fetch_array($result, MYSQL_ASSOC);
...
if( count($_POST)>0 ){ // 更新公告信息
  ...
  if( !$avatar_path ){
    $avatar_path = $arr_img['img'];
  }

  $update_sql = "update home_imgs set title = '{$_POST['title']}',
                          img = '{$avatar_path}'
            where id = {$_POST['id']} ";
  ...
  <div class="public-content">
    ...
    <div class="public-content-cont">
      <form method="post" action="" name="form1" enctype="multipart/form-data">
        <input type="hidden" name="id" value="<?php echo $arr_img['id']?>">

        <div class="form-group">
          <label>标题:</label>
          <input type="text"  class="form-input-txt"  name="title" value="<?php echo $arr_img['title']?>" required placeholder="姓名" style="width: 150px" />
        </div>
        <div class="form-group">
          <label>图片:</label>
          ...
          <?php
          if ($arr_img['img'] ){
            echo "原图片：<a href='{$arr_img['img']}' target='_blank'><img src='{$arr_img['img']}' width='100px' height='100px'></a>";
          }
          ?>
        </div>
